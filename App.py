...
def init_db():
    with get_db_connection() as conn:
        ...
        # Migration: Check and rename columns if old database structure exists
        ...
        # 5. Financial Statements Table (New Feature)
        c.execute('''
...
menu = [
    ...
    "📑 Balance Sheet & Financial Statements", # NAYA OPTION
...
# 6. NEW FEATURE: Balance Sheet & Financial Statements
    ...
    if not client_rows:
        ...
        # Check existing financial statement data
        with get_db_connection() as conn:
            c = conn.cursor()
        ...
        # Defaults
        gross_turnover = fs[3] if fs else 0.0
        gross_profit = fs[4] if fs else 0.0
        net_profit = fs[5] if fs else 0.0
        op_capital = fs[6] if fs else 0.0
        cap_add = fs[7] if fs else 0.0
        drawings = fs[8] if fs else 0.0
        tot_assets = fs[10] if fs else 0.0
        tot_liab = fs[11] if fs else 0.0
        exempt_inc = fs[12] if fs else 0.0
        ded_80c = fs[13] if fs else 0.0
        other_ded = fs[14] if fs else 0.0
        
        st.markdown("### 📊 Enter Financial Year Details")
        
        t1, t2, t3, t4 = st.tabs(["💰 Profit & Loss Account", "🏦 Capital Account", "🏛️ Balance Sheet Summary", "🧮 Computation of Taxable Income"])
        ...
        with t1:
            ...
            with col_p3:
                net_profit = st.number_input("Net Profit (₹)", min_value=0.0, value=float(net_profit), step=1000.0)
        ...
        with t2:
            ...
            with col_c2:
                cap_add = st.number_input("Capital Addition / Introduction (₹)", min_value=0.0, value=float(cap_add), step=1000.0)
            with col_c3:
                drawings = st.number_input("Drawings / Personal Expense (₹)", min_value=0.0, value=float(drawings), step=1000.0)
                
            # Auto Calc Closing Capital
        ...
        with t4:
            st.write("#### Income Tax Computation (Computation Sheet)")
            col_t1, col_t2, col_t3 = st.columns(3)
            with col_t1:
                exempt_inc = st.number_input("Exempt Income (Agricultural/PPF etc) (₹)", min_value=0.0, value=float(exempt_inc), step=1000.0)
            with col_t2:
                ded_80c = st.number_input("Section 80C Deductions (₹)", min_value=0.0, value=float(ded_80c), step=1000.0)
            with col_t3:
                other_ded = st.number_input("Other Chapter VI-A Deductions (80D etc) (₹)", min_value=0.0, value=float(other_ded), step=1000.0)

            gross_total_inc = net_profit - exempt_inc
            taxable_income = max(0.0, gross_total_inc - ded_80c - other_ded)
            st.info(f"📊 **Gross Total Income:** ₹{gross_total_inc:,.2f} | **Taxable Income:** ₹{taxable_income:,.2f}")

        st.markdown("---")
        if st.button("💾 Save Financial Statements Data"):
            ...
            with get_db_connection() as conn:
                ...
                c.execute('''
                    INSERT INTO financial_statements (
                        ...
                        total_assets, total_liabilities, exempt_income, deductions_80c, 
                        other_deductions, taxable_income, created_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ...
                ''', (
                    ...
                    exempt_inc, ded_80c, other_ded, taxable_income, today_str
                ))
                conn.commit()
            st.success("✅ Financial Statement & Tax Computation details saved successfully!")
        ...
        if recs:
            ...
            if st.button("❌ Delete Client Completely"):
                ...
                with get_db_connection() as conn:
                    ...
                    c.execute("DELETE FROM financial_statements WHERE client_id = ?", (cid,))
